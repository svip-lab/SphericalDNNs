...
from sconv.module.sconvlstm import ConvLSTM
...
class SconvLSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm_model = ConvLSTM(input_size=(32, 64),
                 input_dim=64,
                 hidden_dim=[64, 64],
                 kernel_size=(3, 3),
                 num_layers=2,
    ...
    def forward(self, images, lasts):
        ...
        for idx_frame in range(images.size(1)):
            ...
            x = th.cat([image, last], dim=1)
            #print(x.shape, images.shape, image.shape)
            #exit()
            c1 = self.conv1(x)
            b1 = self.bn1(c1)
            r1 = self.relu1(b1)
            ...
            r6u = self.up3(sal_feat)
            #print(r6u.shape, r1.shape)
            #exit()
            c7 = self.conv7(th.cat([r6u, r1], dim=1))

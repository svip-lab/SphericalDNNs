...
from sconv.module.sconvlstm import ConvLSTM as SConvLSTM
from sconv.module.convlstm import ConvLSTM

from opts import opts
opt = opts().parse()

class SphericalConvLSTM(nn.Module):
    def __init__(self):
        super(SphericalConvLSTM, self).__init__()
        ...
        self.lstm_model = SConvLSTM(input_size=(8, 16),
                 input_dim=256,
                 hidden_dim=[256, 256],
                 kernel_size=(3, 3),
                 num_layers=2,
    ...
    def forward(self, images, lasts):
        sal_pred_list = []

        for idx_frame in range(images.size(1)):
            image = images[:, idx_frame, :]#.unsqueeze(0)
            last = lasts[:, idx_frame, :]
            ...
            image_feat = r4.unsqueeze(1)
            if idx_frame == 0:
                sal_feat, hidden_state = self.lstm_model(image_feat)
            else:
                sal_feat, hidden_state = self.lstm_model(image_feat, hidden_state)
            #print( len(hidden_state)) 
            #exit()
            #y_2, hidden_state = self.stacked_lstm(image_feat, (hidden_state[0], hidden_state[1]))
            sal_feat = sal_feat[-1]
            sal_feat = sal_feat.squeeze(1)

            r4u = self.up1(sal_feat)
            ...
            sal_pred_list.append(c7)

            #th.cuda.empty_cache()

        return sal_pred_list
